...
| 항목 | 값 |
| --- | --- |
| `main` | `7e6cfee` — PR #68(#67 리뷰 후속 잔손질 3건 · E-112) 병합. CI · Deploy success |
| 배포 | `main` 병합 → CI 통과 → `deploy.yml`이 EC2로 자동 배포. **EC2는 프리티어**(README 배포 문단) |
| 테스트 | 전체 빌드 575건(PR #68 시점), DB 통합 포함. CI는 `RUN_DB_INTEGRATION_TESTS=true`로 돈다 |
| 열린 PR | **#63**(#58 XLSX 스트리밍 카운트 + 힙 명시) — 리뷰 대기 |
| 열린 이슈 | **#58**(→ PR #63) 하나뿐이다. 9/9 저녁에 정돈했다 — #60 · #66 · #57을 #67로 통합해 PR #68로 닫았고, #49는 닫아 06 R33으로만 추적한다(06 v2.52 · v2.56) |
| docs | 01 v2.36 · 05 v2.36 · 06 v2.56. 06이 정본 진행표다 |
