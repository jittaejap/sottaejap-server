...
public class OnboardingServiceImpl implements OnboardingService {
     ...
     * 기간 전체를 읽는다 — 균등 간격으로 고르려면 몇 건인지 먼저 알아야 한다. 온보딩 CSV는 개인의 두 달치라
     * 페이지를 나눌 크기가 아니다.
     */
    @Override
    ...
    private static List<CandidateView> sample(List<Transaction> transactions, int size) {
        int stride = Math.max(1, transactions.size() / size);
        List<CandidateView> sample = new ArrayList<>();
        for (int i = 0; i < transactions.size() && sample.size() < size; i += stride) {
            sample.add(CandidateView.from(transactions.get(i), ReasonCode.ONBOARDING_SAMPLE,
